...
class RealAjaxUploader
{
	...
	function __construct($deny_ext=array())
	{
		...
		$this->actual_file_name = isset($_REQUEST['ax-file-name']) ? $_REQUEST['ax-file-name']:$_FILES['ax_file_input']['name'];
		$this->file_name = isset($_REQUEST['ax-file-name']) ? $_REQUEST['ax-file-name']:$_FILES['ax_file_input']['name'];
		$this->file_name = uniqid('',true)."_".$this->file_name;
	...
	{
		$this->header();
		echo json_encode(array('actual_file_name'=>$this->actual_file_name,'name'=>$this->file_name, 'size'=>$this->file_size, 'status'=>$status,'info'=>$msg, 'more'=>$extra_info));
		die();
	}
